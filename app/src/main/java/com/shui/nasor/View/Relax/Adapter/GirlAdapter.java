...
public class GirlAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<GirlEntity.ShowapiResBodyBean.NewslistBean> mData;
    ...
    private int mHeaderCount = 0;//头布局个数
    private int mFooterCount = 1;//尾布局个数

    public GirlAdapter(List<GirlEntity.ShowapiResBodyBean.NewslistBean> mData, Context mContext) {
        this.mData = mData;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_TYPE_BOTTOM || viewType == ITEM_TYPE_HEADER) {
            ...
            return new ImgJokerHolder(mInflater.inflate(R.layout.holder_item_girl_content,parent,false));
        }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        {
            GirlEntity.ShowapiResBodyBean.NewslistBean bean=mData.get(position-mHeaderCount);
            ImageLoader.load(mContext,bean.getPicUrl(),((ImgJokerHolder) holder).girlImage);
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    View shareView = view.findViewById(R.id.girlIVPic);
                    if (listener!=null)
                    {
            ...
            resizeItemView(((ImgJokerHolder) holder).girlImage,getScaleType(position));
        }
        else {
            ((HeadFootHolder) holder).imgJokerTVPage.setText("正在加载更多~~");
        }
    }
